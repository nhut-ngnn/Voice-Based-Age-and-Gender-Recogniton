...
def get_audio_features(path, extra_features=[""]):
    ...
    if "constract" in extra_features:
        constract = librosa.feature.delta(mfcc_features)
        features.extend(constract)
...
def mfcc_to_delta(mfcc_coef, d):
    ...
    mask = np.ones(vx.shape, dtype=np.bool_)
    mask[: d * 2, :] = False
